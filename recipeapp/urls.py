...
urlpatterns = [
    ...
    re_path('api/recipes/(?P<pk>[0-9]+)', views.get_recipe),
    re_path('api/recipes/delete/(?P<pk>[0-9]+)', views.delete_recipe),
    path('api/recipes/search', views.search_recipes),
    re_path('api/recipes/update/(?P<pk>[0-9]+)', views.update_recipe),
    re_path('api/recipes/(?P<pk>[0-9]+)/ratings/', views.RatingViewSet.as_view({'get': 'list', 'post': 'create'}),name='recipe_ratings'),
]
